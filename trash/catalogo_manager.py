...
class CatalogoManager:
    ...
    def cargar_autores(self):
        ...
        if source == 'local':
            rows = self.query_local("SELECT id, nombreAutor FROM core_autores ORDER BY nombreAutor COLLATE NOCASE")
        else:
            rows = self.query_turso("SELECT id, nombreAutor FROM core_autores ORDER BY nombreAutor COLLATE NOCASE")
    ...
    def cargar_editoriales(self):
        ...
        if source == 'local':
            rows = self.query_local("SELECT id, descriEditorial FROM core_editoriales ORDER BY descriEditorial COLLATE NOCASE")
        else:
            rows = self.query_turso("SELECT id, descriEditorial FROM core_editoriales ORDER BY descriEditorial COLLATE NOCASE")
    ...
    def guardar_libro(self, destino):
        ...
        try:
            ...
            if 'UNIQUE constraint failed' in error_msg or 'unique' in error_msg.lower():
                messagebox.showerror("Error", "El EAN ya existe en la base de datos. Por favor, usa un EAN diferente.")
            else:
                messagebox.showerror("Error", f"No se pudo guardar el libro: {error_msg}")
    ...
    def buscar_autores(self):
        ...
        if search_term:
            sql = """
                ...
                ORDER BY a.nombreAutor COLLATE NOCASE
            """
    ...
    def crear_nuevo_autor(self):
        ...
        ttk.Label(form_frame, text="Observaciones:").grid(row=3, column=0, sticky='ne', padx=5, pady=5)
        observaciones_text = scrolledtext.ScrolledText(form_frame, width=40, height=4)
        observaciones_text.grid(row=3, column=1, columnspan=2, padx=5, pady=5)
        ...
        def guardar():
            ...
            source = self.source_var.get()
            sql = "INSERT INTO core_autores (nombreAutor, enlaceWiki, enlaceWiki2, observaciones, created, updated) VALUES (?, ?, ?, ?, datetime('now'), datetime('now'))"
            
            if source == 'local':
                result = self.query_local(sql, (nombre, wiki1, wiki2, observaciones))
            else:
                result = self.query_turso(sql, [nombre, wiki1, wiki2, observaciones])
    ...
    def editar_autor_seleccionado(self):
        ...
        observaciones_text = scrolledtext.ScrolledText(form_frame, width=40, height=4)
        observaciones_text.grid(row=3, column=1, columnspan=2, padx=5, pady=5)
        if autor.get('observaciones'):
            observaciones_text.insert('1.0', autor['observaciones'])
        ...
        def guardar():
            ...
            source = self.source_var.get()
            sql = "UPDATE core_autores SET nombreAutor = ?, enlaceWiki = ?, enlaceWiki2 = ?, observaciones = ?, updated = datetime('now') WHERE id = ?"
            
            if source == 'local':
                result = self.query_local(sql, (nombre, wiki1, wiki2, observaciones, autor_id))
            else:
                result = self.query_turso(sql, [nombre, wiki1, wiki2, observaciones, autor_id])
    ...
    def buscar_editoriales(self):
        ...
        if search_term:
            sql = """
                ...
                ORDER BY e.descriEditorial COLLATE NOCASE
            """
            params = (f'%{search_term}%',)
        else:
            sql = """
